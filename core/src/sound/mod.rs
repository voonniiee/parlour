...
use symphonia::{core::{audio::SampleBuffer, codecs::{CodecRegistry, Decoder, DecoderOptions}, formats::{FormatOptions, FormatReader}, io::{MediaSource, MediaSourceStream, MediaSourceStreamOptions}, meta::MetadataOptions, probe::Hint}, default::{get_codecs, get_probe}};
...
impl Sound {
	...
	let mut decoder = codecs.make(&track.codec_params, &DecoderOptions::default()).unwrap();
	...
	for chunk in data.chunks(dft_window_len) {
	    ...
	    fft.process_with_scratch(&mut chunk, &mut output, &mut scratch).unwrap();
	...
	let format = probe.format(&Hint::new(), stream, &FormatOptions::default(), &MetadataOptions::default()).unwrap();
